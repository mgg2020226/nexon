...
import { LazyMotion, m, domAnimation, type Variants } from 'framer-motion';
...
function AnimatedGroup({ children, className, variants, preset }: AnimatedGroupProps) {
  ...
  return (
    <LazyMotion features={domAnimation}>
      ...
      >
        {(React.Children.toArray(children) as React.ReactElement[]).map((child) => (
          <m.div key={child.key} variants={itemVariants}>
            {child}
          </m.div>
        ))}
      </m.div>
